...
def coverage(c):
    ...
    """
    c.run("coverage run --source gm_auth_sdk runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/gm-auth-sdk.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ gm_auth_sdk")
...
def lint(c):
    ...
    """
    c.run("flake8 gm-auth-sdk tests")
...
def release(c, bumpsize=""):
    ...
    c.run(
        'git tag -a {version} -m "New version: {version}"'.format(
            version=gm_auth_sdk.__version__
        )
    )
    c.run("git push --tags")
